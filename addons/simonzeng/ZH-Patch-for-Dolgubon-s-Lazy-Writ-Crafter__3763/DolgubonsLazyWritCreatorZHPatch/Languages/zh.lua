...
function WritCreater.langTutorial(i) 
	local t = {
		[5]="你还应该了解的这些。\n/dailyreset 是一个斜杠命令，可以查看距离服务器重置每日委托任务还有多长时间。",
...
local function masterWritEnchantToCraft (pat,set,trait,style,qual,mat,writName,Mname,generalName)
	...
	local partialString = zo_strformat("用 <<t:2>> 制作具有 <<t:3>> 特质，<<t:4>> 风格，品质为 <<t:5>> 的 CP150 <<t:6>> 套装 <<t:1>>",pat,set,trait,style,qual,mat)
...
local function runeMissingFunction (ta,essence,potency)
	...
	for i = 1, #missing do
		if i ==1 then
			--O_CachedStrFormat("<<C:1>>",missing[i])
			text = "|cff3333无法制作雕文\n缺少: "..proper(missing[i])
		else
...
WritCreater.strings["name"]="Lazy Writ Crafter 委托制作"
WritCreater.strings["displayName"]="|c8080FF Dolgubon's Lazy Writ Crafter 委托制作|r"
...
WritCreater.strings["notEnoughSkill"]	= "你的制作技能未达到制作装备所需的等级"
...
WritCreater.strings["craftIncomplete"] = "|cf6000制作未完成，需要更多的材料。|r"
...
WritCreater.optionStrings["alchemy tooltip"] = "开/关炼金插件（仅限从银行取回）"
WritCreater.optionStrings["provisioning"]= "烹饪"
WritCreater.optionStrings["provisioning tooltip"] = "开/关烹饪插件（仅限从银行取回）"
...
WritCreater.optionStrings["writ grabbing"]= "取回所需物品"
WritCreater.optionStrings["writ grabbing tooltip"] = "从银行取回委托任务所需物品（如：奈恩根、塔等）。"
...
WritCreater.optionStrings["loot output"]= "高价值的奖励提醒"
...
WritCreater.optionStrings['autoCloseBankTooltip'] = "如果有要从银行取回的材料，则自动打开和退出银行对话框"
WritCreater.optionStrings['despawnBanker']= "解散银行家（取回）"
WritCreater.optionStrings['despawnBankerTooltip'] = "取回需要的材料后自动解散银行家"
WritCreater.optionStrings['despawnBankerDeposit'] = "退出并解散银行家（寄存）"
WritCreater.optionStrings['despawnBankerDepositTooltip'] = "寄存物品后解散解散银行家"
...
WritCreater.optionStrings['hireling behaviour choices']	= { "无","获取附件并删除", "获取附件"}
...
WritCreater.optionStrings['sameForALlCraftsTooltip']	= "所有制作类别采用相同的选项"
...
WritCreater.optionStrings["rewardChoices"]= {"不自动处理","寄存银行","标记为废品", "摧毁", "分解"}
