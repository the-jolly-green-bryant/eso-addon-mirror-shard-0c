...
function WritCreater.loadStatusBar()
	...
	StatusBar:SetAnchor(TOPRIGHT, GuiRoot, TOPLEFT, WritCreater:GetSettings().statusBarX, WritCreater:GetSettings().statusBarY)
	if WritCreater:GetSettings().StatusBarIcons then
		StatusBar:SetDimensions(190, 37)
	else
		StatusBar:SetDimensions(120, 37)
	end
	if WritCreater:GetSettings().StatusBarBackdropColor then
		DolgubonsLazyWritStatusBackdrop:SetCenterColor(255, 255, 255, 0)
	else
		DolgubonsLazyWritStatusBackdrop:SetCenterColor(255, 255, 255, 255)
	end		
end
