...
AG.version = 'v6.4.1 beta3'
AG.author = 'AlphaLemming & mesota'
AG.init = false
AG.pendingSet = -1
AG.previousSet = nil
...
AG.account = {}
AG.account_defaults = {
	option = {true, true, true, true, true, true, true, true, true, true, true, false, true, true, AG.CHOICE_SHOW_ITEM_LEVEL_NEVER
--> BAERTRAM - FCOIS
        ,false
--< BAERTRAM - FCOIS
		,false
...
function AG.DrawSkillBarButtons(nr)
    ...
    p:SetHandler('OnMouseDown',
        function(self,button,ctrl,alt,shift)
            if button == 2 then
                AG.DrawMenu(self, {true, (MENU.copySourceSetId and MENU.type == AG.MENU_TYPE_SKILLS), true, true, false, false})
                MENU.nr = nr
    ...
    p:SetHandler('OnMouseDown',
        function(self,button,ctrl,alt,shift)
            if button == 2 then 
                ...
                AG.DrawMenu(self, {true, (MENU.copySourceSetId and MENU.type == AG.MENU_TYPE_GEAR), true, true, isBanking, isBanking})
                MENU.nr = nr
...
--    local profileCombo = ZO_ComboBox_ObjectFromContainer(AG_PanelProfilePanel:GetNamedChild("ProfileSelect"))
-- 	AG.SetupProfileCombo(profileCombo)
...
function AG.DrawBuild(nr)
    ...
    p:SetDimensions(panelWidth-22, 30)
    p:SetFont('AGFont')
    p:SetColor(1,1,1,1)
    ...
    p:SetDimensions(panelWidth-8, 30)
...
--[[
        if Id64ToString(GetItemUniqueId(BAG_WORN, targetSlot)) ~= AG.setdata[nr].Gear[slotIndex].id then
            local sourceBag, sourceBagSlot = AG.GetItemFromBag(AG.setdata[nr].Gear[slotIndex].id)

            if sourceBagSlot then 
                -- CallSecureProtected("RequestMoveItem", bag, sourceBagSlot, BAG_WORN, targetSlot, 1)
                -- d("Equip. Source-Bag: "..sourceBag.." -Slot: "..sourceBagSlot.." Target-Slot: ".. targetSlot)

                table.insert(AG.Jobs, {AG.JOB_TYPE_EQUIP_GEAR, sourceBag, sourceBagSlot, targetSlot}) 
...
--] ]
                ...
                d(ZOSF(L.NotFound,AG.setdata[nr].Gear[slotIndex].link)) 
            end
        end
--]]
...
function AG.GetSkill(abilityId)
    ...
    local skillType, skillLine, abilityIndex = GetSkillAbilityIndicesFromProgressionIndex(progressionIndex)
    if skillType > 0 then 
...
function AG.LoadSkill(nr, slot)
    ...
    if skillID ~= currentSkillID then
        ...
        if newAbilityIndex ~= 0 and currentAbilityIndex ~= newAbilityIndex then 
            ...
            CallSecureProtected('SelectSlotAbility', newAbilityIndex, slot+2) 
        end
    end
end
...
-- computes a map item-Id -> assigend builds for all items used in builds
...
--> BAERTRAM - FCOIS
function AG.SetupFCOISSetMarkerIconCombo(dropdown, setnr)
    local currentSetData = AG.setdata[setnr].Set
    currentSetData["FCOIS"] = currentSetData["FCOIS"] or {}
    dropdown:ClearItems()
    ...
    local function OnEntrySelected(_, _, entry)
		...
		local oldIconId = currentSetData["FCOIS"]["icon"] or 0
		local newIconId = entry.FCOISgearMarkerIcon
		
		if oldIconId ~= newIconId then
			local usageMap = AG.ComputeItemUsageMap()
			local AGplugFCOIS = AG.plugins.FCOIS

			AGplugFCOIS.onChangeBuildIcon(usageMap, setnr, oldIconId, newIconId)
			AGplugFCOIS.updateInventoryList()
			
	        --Set the new set marker icon
			currentSetData["FCOIS"]["icon"] = newIconId
		end
		
		--[[
		
		
        AG.setdata[setnr].Set["FCOIS"]["iconRemove"] = AG.setdata[setnr].Set["FCOIS"]["iconRemove"] or {}
        
		--Add the FCOIS set marker, which is currently set, to the list of icons which should be removed on next
        --inventory open
        if currentSetData["FCOIS"]["icon"] ~= nil and currentSetData["FCOIS"]["icon"] ~= 0 then
            --Icon was really changed?
            if currentSetData["FCOIS"]["icon"] ~= entry.FCOISgearMarkerIcon then
                AG.setdata[setnr].Set["FCOIS"]["iconRemove"][currentSetData["FCOIS"]["icon"] ] = true
            end
        end
		
        --Set the new set marker icon
        AG.setdata[setnr].Set["FCOIS"]["icon"] = entry.FCOISgearMarkerIcon
        --Current new icon is in the remove list? Remove the "remove" entry again
        if entry.FCOISgearMarkerIcon ~= nil and currentSetData["FCOIS"]["iconRemove"][entry.FCOISgearMarkerIcon] then
            AG.setdata[setnr].Set["FCOIS"]["iconRemove"][entry.FCOISgearMarkerIcon] = nil
        end
    ...
    -- Add "None" entry
    local noFCOISGearMarkerIconEntry = ZO_ComboBox:CreateItemEntry(L.NoFCOISGearMarkerIconEntry, OnEntrySelected)
    ...
    if gearMarkerIconsList ~= nil and gearMarkerIconsValuesList ~= nil then
        ...
        for FCOISgearMarkerIcon = 1, #gearMarkerIconsList do
            ...
            dropdown:AddItem(entry, ZO_COMBOBOX_SUPRESS_UPDATE)
            if currentSetData["FCOIS"]["icon"] == gearMarkerIconsValuesList[FCOISgearMarkerIcon] then
                defaultEntry = entry
            end
        end
    end
...
function AG.SetupProfileCombo()
    ...
    for profileId = 1, MAX_PROFILES do
        local entry = ZO_ComboBox:CreateItemEntry(AG.setdata.profiles[profileId].name, OnProfileSelected)
...
function AG.LoadSetInternal(nr)
    ...
    -- set outfit
    AG.LoadOutfit (AG.setdata[nr].Set.outfit)
...
function AG.DoTransfer(sourceBagId, sourceBagSlot)
    ClearCursor()
    local result = CallSecureProtected("PickupInventoryItem", sourceBagId, sourceBagSlot, 1)
    if result then
...
function AG.WithdrawItem(sourceBagId, sourceBagSlot)
    
    if DoesBagHaveSpaceFor(BAG_BACKPACK, sourceBagId, sourceBagSlot) then
        AG.DoTransfer(sourceBagId, sourceBagSlot)
    else
...
function AG.DepositItem(sourceBagId, sourceBagSlot)
...
function AG.EquipItem(gear, targetSlot)
	...
	-- anything to change?
    if Id64ToString(GetItemUniqueId(BAG_WORN, targetSlot)) ~= gear.id then
...
function AG.MoveGearSet(deposit, nr)
    if not nr then return end
    AG.ClearAvailableEquipmentCache()
    ...
    for slotIndex = 1, #SLOTS do 
        ...
        if id and id ~= 0 then
            ...
            if sourceBagSlot ~= nil then
                ...
                if deposit then
                    -- only items in backpack can be deposited
                    if sourceBag == BAG_BACKPACK then
                        table.insert(AG.Jobs, {AG.JOB_TYPE_DEPOSIT_GEAR, sourceBag, sourceBagSlot}) 
                    elseif sourceBag == BAG_WORN then
                        local link = AG.setdata[nr].Gear[slotIndex].link
                        d(ZOSF(L.CurrentlyEquipped, link)) 
                    end
                else
                    -- only items not worn and not in backpack can be withdrawn
                    if sourceBag ~= BAG_BACKPACK and sourceBag ~= BAG_WORN then
                        table.insert(AG.Jobs, {AG.JOB_TYPE_WITHDRAW_GEAR, sourceBag, sourceBagSlot}) 
                    end
                end
            end
        end
...
function AG.HandleOnUpdate()
    if AG.init and #AG.Jobs > 0 then
        ...
        if CurrentTime >= AG.NextEventTime or AG.NextEventTime == 0 then
			...
			if mustWait then
				-- next event in 250ms from now
				AG.NextEventTime = CurrentTime + 100
			end
        end
    end
end
...
function AG.UpdateEditPanel(nr)
    ...
    c = ZO_ComboBox_ObjectFromContainer(AG_PanelSetPanelScrollChildEditPanelSetSettings:GetNamedChild("FCOISSetMarkerIconDropdown"))
    local AGplugFCOIS = AG.plugins.FCOIS
    if AGplugFCOIS.isAddonReady() and AGplugFCOIS.isFCOISMarkerIconsEnabled() then
        c.m_container:SetHidden(false)
        AG.SetupFCOISSetMarkerIconCombo(c, nr)
    else
        c.m_container:SetHidden(true)
    end
    --< BAERTRAM - FCOIS
end
...
function AG.UpdateInventorySingleItem()
...
function AG.SetupInventoryCallback()
	...
	local function UpdateItemMarker(c, slot)
		--> BAERTRAM - FCOIS
		local updateFCOISInv = false
		local isFCOISAddonReady = AGplugFCOIS.isAddonReady()
		local isFCOISMarkerIconsEnabled = AGplugFCOIS.isFCOISMarkerIconsEnabled()
		local markGearWithAG = AG.isMarkSetItemsInBag()
		local markGearWithFCOIS = isFCOISAddonReady and isFCOISMarkerIconsEnabled
		local markGearWithAGAndOrFCOIS = markGearWithAG or markGearWithFCOIS
		--d("[AG]SetupInventoryCallback \"".. tostring(inv) .."\", isFCOISAddonReady: " .. tostring(isFCOISAddonReady) ..", isFCOISMarkerIconsEnabled: " .. tostring(isFCOISMarkerIconsEnabled) .. ", markGearWithAG: " ..tostring(markGearWithAG) .. ", markGearWithFCOIS: " .. tostring(markGearWithFCOIS)  .. ", markGearWithAGAndOrFCOIS: " ..tostring(markGearWithAGAndOrFCOIS))
		--< BAERTRAM - FCOIS
		...
		if isFCOISAddonReady or markGearWithAG then

	        local slotInfo = c.dataEntry.data or nil
	        if not slotInfo then return end

			local uid = Id64ToString(GetItemUniqueId(slotInfo.bagId, slotInfo.slotIndex))
			...
			for nr = 1, MAXSLOT do
				local gearSet = setData[nr].Set.gear
				if gearSet > 0 then
					for slot = 1,14 do
						if setData[gearSet].Gear[slot].id == uid then
							table.insert(buildIds, nr)
							break
						end
					end
				end
				
				-- shortcut for AG only
				if #buildIds > 0 and not isFCOISAddonReady then
					break
				end
			end
		
			-- item is part of at least one gear-set
			if #buildIds > 0 and markGearWithAG then
				ShowAGMarker(c)
			end

			--> BAERTRAM - FCOIS
			--Addon FCOItemSaver is loaded and the plugin to set the marker icons is enabled too?
			if isFCOISAddonReady then
				--Mark the inventory item with the FCOIS marker icon for the gear set items now
				-- AGplugFCOIS.markInventoryItem(c, isFCOISMarkerIconsEnabled, true, buildIds)
			end
			--< BAERTRAM - FCOIS
		end
	end
...
function AG.SetConnect(parent,mode)
    local col, p = {'green','grey'}, {AG.MODE_SKILL, AG.MODE_SKILL, AG.MODE_GEAR}
    for nr = 1, MAXSLOT do
        WM:GetControlByName('AG_Selector_'..p[parent]..'_'..nr):SetNormalTexture('AlphaGear/asset/'..col[mode]..'.dds')
    end
end
...
function AG.UpdateChargedWeapons(isBackPair)
    trace('AG.UpdateChargedWeapons called')
...
function AG.ShowIconMenu(c, button, bar)
    SELECTBAR = bar
...
-- 1: copy, 2: paste, 3: insert equipped, 4: clear, 5: deposit, 6: withdraw
function AG.MenuAction(nr)
    ...
    elseif nr == 5 then
        -- deposit
        if MENU.type == AG.MENU_TYPE_GEAR then 
            AG.MoveGearSet(true, MENU.nr)
            AG.UpdateUI(MENU.nr,MENU.nr)
        end
    elseif nr == 6 then
        -- withdraw
        if MENU.type == AG.MENU_TYPE_GEAR then 
            AG.MoveGearSet(false, MENU.nr)
            AG.UpdateUI(MENU.nr,MENU.nr)
        end
    end
end
...
function AG:Initialize()
    ...
    EM:RegisterForEvent('AG4', EVENT_INVENTORY_SINGLE_SLOT_UPDATE, AG.ClearAvailableEquipmentCache)
