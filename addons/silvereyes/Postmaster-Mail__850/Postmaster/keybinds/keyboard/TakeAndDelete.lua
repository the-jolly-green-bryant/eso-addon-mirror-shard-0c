...
function TakeAndDelete:CanTake(mailData)
    ...
    -- mailData is not guaranteed to be valued at this point.
    if not mailData or not mailData.mailId or type(mailData.mailId) ~= "number" then
        addon.Utility.Debug("TakeAndDelete:CanTake(" .. tostring(mailData and mailData.mailId)
            .. ") returning false due to a bad mailId", debug)
        return false
    end

    -- Exclude any items that we've already read attachments for that we know
