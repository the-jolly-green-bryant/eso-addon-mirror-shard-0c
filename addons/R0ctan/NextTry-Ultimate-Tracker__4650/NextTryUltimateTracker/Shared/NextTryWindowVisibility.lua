...
if not NextTryShared.WindowVisibility or (NextTryShared.WindowVisibility.version or 0) < MODULE_VERSION then
    local V = {}
    V.version = MODULE_VERSION

    function V.GetCurrentSceneName()
    ...
    function V.IsHudSceneActive()
        local sceneName = V.GetCurrentSceneName()
        if sceneName then
    ...
    function V.IsSettingsPreviewActive(addon)
    ...
    function V.GetVisibilityDecision(addon)
        ...
        if V.IsSettingsPreviewActive(addon) then
            return true, "settings preview"
        end

        local sceneName = V.GetCurrentSceneName() or "unknown"
        if not V.IsHudSceneActive() then
    ...
    function V.ShouldShowWindow(addon)
        return select(1, V.GetVisibilityDecision(addon))
    end

    function V.ApplyVisibility(addon, window)
        if not addon then return end
        window = window or addon.window or addon.container
        if not window then return end
        window:SetHidden(not V.ShouldShowWindow(addon))
    end

    function V.ScheduleApplyVisibility(addon, delayMs, applyFn)
        local function apply()
            if type(applyFn) == "function" then
                applyFn()
            else
                V.ApplyVisibility(addon)
            end
        end

        if not addon or not EVENT_MANAGER or not addon.name then
            if zo_callLater then
                zo_callLater(apply, delayMs or 200)
            else
                apply()
            end
            return
    ...
    function V.RegisterSceneCallbacks(addon, callbackName, applyFn)
        if not addon or addon.windowVisibilitySceneCallbacksRegistered or addon.sceneVisibilityCallbacksRegistered then return end
        addon.windowVisibilitySceneCallbacksRegistered = true
        addon.sceneVisibilityCallbacksRegistered = true
        ...
        SCENE_MANAGER:RegisterCallback(callbackName or "SceneStateChanged", function(scene, oldState, newState)
            if newState == SCENE_SHOWN or newState == SCENE_HIDDEN then
                applyNow()
                V.ScheduleApplyVisibility(addon, 200, applyFn)
            end
        end)
    end

    function V.RegisterLAMPreviewCallbacks(addon, isOwnPanelFn, setPreviewFn)
        if not addon or addon.windowVisibilityLAMCallbacksRegistered or addon.settingsPreviewCallbacksRegistered or not CALLBACK_MANAGER then return end
        addon.windowVisibilityLAMCallbacksRegistered = true
        addon.settingsPreviewCallbacksRegistered = true
    ...
    function V.ValidatePosition(position, defaults)
    ...
    NextTryShared.WindowVisibility = V
end
